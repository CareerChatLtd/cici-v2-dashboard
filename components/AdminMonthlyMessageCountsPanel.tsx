...
export const AdminMonthlyMessageCountsPanel = ({dateRange}: Props) => {
    ...
    useEffect(() => {
        ...
        fetch(`/api/monthly-message-counts-all-tenants?${params}`)
            .then(res => res.json())
            .then(data => {
                setData(data);
                setLoading(false);
    ...
    const noData = !data || data.length === 0 || data.reduce((acc, {totalMessages}) => acc + totalMessages, 0) === 0;

    return (
        <div className="bg-white p-3 h-full">
            <div className="flex flex-row justify-between items-center mb-3">
                <h3 className="text-sm font-bold text-gray-600">Messages sent by users (All Tenants)</h3>
            </div>
            <div>
                {loading ? <Loading/>
